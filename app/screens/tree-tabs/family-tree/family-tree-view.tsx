...
import { describeFamilyStep, lineageIds, scopeIds, relationshipPath, type TreeScope } from '../../../../components/tree-exploration';
...
export function FamilyTreeView({
  ...
  const [scope, setScope] = useState<TreeScope>('full');
  ...
  useEffect(() => {
    setSelectedId(null); setRootId(null); setScope('full'); setCollapsed([]);
  ...
  const scoped = useMemo(() => scopeIds(focusRoot, relationships, scope), [focusRoot, relationships, scope]);
  ...
  const changeScope = (next: TreeScope) => { setScope(next); setRootId(selectedId ?? focusRoot); setCollapsed([]); setTraceFrom(null); setTraceTo(null); setFocusRequest(undefined); };
    ...
    <View style={[styles.visualisationTabContainer, { backgroundColor: theme.colors.background }]}>
      ...
      {people.length > 0 ? (
        <FamilyTreeCanvas
          ...
          viewportStorageKey={`${selectedTree.id}:${scope}:${focusRoot}:${collapsed.join(',')}`}
          ...
          searchControls={<View style={{ flexShrink: 1, gap: 2 }}>
            {width < 900 ? <Text variant="labelMedium" style={{ paddingHorizontal: 8, color: theme.colors.primary }}>{t(scope === 'close' ? 'Close family' : scope === 'ancestors' ? 'Ancestors' : scope === 'descendants' ? 'Descendants' : 'Full tree')}</Text> : null}
            <View accessibilityRole="tablist" style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 0 }}>
              {([
                { key: 'close', label: 'Close family', icon: 'account-group-outline' },
                { key: 'ancestors', label: 'Ancestors', icon: 'arrow-up-bold-outline' },
                { key: 'descendants', label: 'Descendants', icon: 'arrow-down-bold-outline' },
                { key: 'full', label: 'Full tree', icon: 'family-tree' },
