...
export interface InviteEmailTemplateData extends BaseTemplateData {
  inviterName: string;
...
export interface PasswordResetEmailTemplateData extends BaseTemplateData {
  recipientName?: string;
  resetUrl: string;
  expiresIn?: string;
}

export interface MagicLinkEmailTemplateData extends BaseTemplateData {
  recipientName?: string;
  signInUrl: string;
  expiresIn?: string;
}

export interface AccountCreatedEmailTemplateData extends BaseTemplateData {
  recipientName?: string;
  loginUrl: string;
...
function mergeBrand(data?: BaseTemplateData) {
  return {
    ...defaultBrand,
    ...data,
  };
...
function buildButton(label: string, url: string, color: string) {
  return `
...
function buildMetadataRows(metadata?: Array<{ label: string; value: string }>) {
  if (!metadata?.length) {
    return '';
  }
...
export function buildInviteEmailTemplate(data: InviteEmailTemplateData): EmailTemplateResult {
...
export function buildPasswordResetEmailTemplate(data: PasswordResetEmailTemplateData): EmailTemplateResult {
  const brand = mergeBrand(data);
  const recipientLabel = data.recipientName?.trim() || 'there';
...
export function buildMagicLinkEmailTemplate(data: MagicLinkEmailTemplateData): EmailTemplateResult {
  const brand = mergeBrand(data);
  const recipientLabel = data.recipientName?.trim() || 'there';
...
export function buildAccountCreatedEmailTemplate(data: AccountCreatedEmailTemplateData): EmailTemplateResult {
  const brand = mergeBrand(data);
  const recipientLabel = data.recipientName?.trim() || 'there';
...
export const lineageTreeEmailTemplates = {
