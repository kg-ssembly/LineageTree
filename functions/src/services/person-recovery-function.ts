...
export async function archivePerson(db: Firestore, treeId: string, personId: string, actorId?: string, transaction?: Transaction) {
  const apply = async (tx: Transaction) => {
    ...
    if (!tree.exists || (actorId && !tree.data()?.editorIds?.includes(actorId))) throw new HttpsError('permission-denied', 'Only a tree editor can remove this person.');
    if (!person.exists) return { ok: true };
    if (person.data()?.treeId !== treeId) throw new HttpsError('permission-denied', 'This person belongs to another tree.');
    if ((person.data()?.treeMembershipIds?.length ?? 1) > 1) throw new HttpsError('failed-precondition', 'This person is shared between trees. Resolve their memberships before deleting them.');
    ...
    if (links.length + parents.length > 400) throw new HttpsError('failed-precondition', 'This person needs administrator-assisted deletion.');
    ...
    parents.filter((p) => p.exists).forEach((p) => tx.update(p.ref, {
      lifeEvents: (p.data()?.lifeEvents ?? []).filter((event: { id: string }) => event.id !== `child-born-${personId}`),
...
export async function restoreDeletedPerson(db: Firestore, actorId: string, treeId: string, personId: string, restoreLinks = true) {
  if (!treeId || !personId || treeId.includes('/') || personId.includes('/')) throw new HttpsError('invalid-argument', 'Choose a deleted person.');
  return db.runTransaction(async (tx) => {
    ...
    if (!tree.exists || tree.data()?.ownerId !== actorId) throw new HttpsError('permission-denied', 'Only the tree owner can restore deleted records.');
    if (!trash.exists || trash.data()?.treeId !== treeId) throw new HttpsError('not-found', 'This deleted record is no longer available.');
    if (current.exists) throw new HttpsError('failed-precondition', 'This person already exists. Refresh the tree.');
    const person = trash.data()!.person as PersonRecord;
    const relationships = (restoreLinks ? trash.data()!.relationships ?? [] : []) as RelationshipRecord[];
    if (relationships.length > 400) throw new HttpsError('failed-precondition', 'This record needs administrator-assisted recovery.');
    ...
    if (otherPeople.some((p) => !p.exists || p.data()?.treeId !== treeId) || existingRelationships.some((r) => r.exists)) {
      throw new HttpsError('failed-precondition', 'A connected person is missing or a relationship has changed. Restore connected people first or ask an administrator to resolve the conflict.');
    }
    if (relationships.length) {
      ...
      const people = [person, ...peopleSnapshot.docs.map((p) => ({ ...p.data(), id: p.id } as PersonRecord))];
      const currentLinks = linksSnapshot.docs.map((r) => ({ ...r.data(), id: r.id } as RelationshipRecord));
      for (const relationship of relationships) {
        const error = validateProposedRelationship({ people, relationships: currentLinks, ...relationship });
        if (error) throw new HttpsError('failed-precondition', `Relationships cannot be restored: ${error} Restore the person only and review their connections.`);
    ...
    tx.create(db.collection('approvalRequests').doc(), {
      ...
      requestedByLabel: tree.data()?.collaborators?.find((c: { userId: string }) => c.userId === actorId)?.displayName || 'Tree owner',
