...
exports.requestTreeAccess = requestTreeAccess;
exports.respondToAccess = respondToAccess;
const https_1 = require("firebase-functions/v2/https");
async function requestTreeAccess(db, actorId, treeId) {
    if (!treeId || treeId.includes('/'))
        throw new https_1.HttpsError('invalid-argument', 'Choose a tree.');
    return db.runTransaction(async (tx) => {
        ...
        if (!tree.exists || data?.discoverable !== true)
            throw new https_1.HttpsError('not-found', 'This tree is not available for access requests.');
        if (data.memberIds?.includes(actorId))
            throw new https_1.HttpsError('already-exists', 'You already have access to this tree.');
...
async function respondToAccess(db, actorId, notificationId, status) {
    if (!notificationId || notificationId.includes('/') || !['accepted', 'rejected', 'dismissed'].includes(status))
        throw new https_1.HttpsError('invalid-argument', 'Choose an access request and response.');
    return db.runTransaction(async (tx) => {
        const ref = db.doc(`notifications/${notificationId}`);
        const notice = await tx.get(ref);
        const n = notice.data();
        if (!n || n.userId !== actorId)
            throw new https_1.HttpsError('permission-denied', 'This request belongs to another user.');
        const cancel = status === 'dismissed';
        if (n.type !== (cancel ? 'tree-access-response' : 'tree-access-request'))
            throw new https_1.HttpsError('invalid-argument', 'Choose an access request.');
        if (n.status !== 'pending')
            return { ok: true };
        const treeRef = db.doc(`trees/${n.sourceTreeId}`);
        const tree = await tx.get(treeRef);
        const data = tree.data();
        if (!data || (!cancel && data.ownerId !== actorId))
            throw new https_1.HttpsError('permission-denied', 'Only the tree owner can respond.');
